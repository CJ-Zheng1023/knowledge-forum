module.exports = {
    QUERY_USER_BY_USERNAME: 'select id,user_name,signature,password,mail,photo from user where user_name=?',
    ...
    UPDATE_USER_BY_USERNAME: 'update user set signature=?,password=?,photo=? where user_name=?',
    QUERY_ALL_TOPIC_BY_USER_ID_COUNT: 'select count(1) as count from topic where user_id=?',
    QUERY_PASSWORD_BY_USERNAME: 'select password from user where user_name=?',
    UPDATE_USER_BY_USERNAME_EXCLUDE_PASSWORD: 'update user set signature=?,photo=? where user_name=?'
