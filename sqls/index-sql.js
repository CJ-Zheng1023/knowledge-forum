module.exports = {
    SELECT_TOPIC_LIST: 'select t.id,title,page_view,reply_num,user_name,category,u.photo from topic t,user u where u.id=t.user_id order by t.create_time desc',
    SELECT_TOPIC_LIST_BY_CATEGORY: 'select t.id,title,page_view,reply_num,user_name,u.photo from topic t,user u where u.id=t.user_id and category=? order by t.create_time desc',
